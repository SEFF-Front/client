...
function PublishedCourses(){
  ...
  let diplayedArr = publishedCourses;
  if(search){
    diplayedArr=publishedCourses.filter((el)=>el?.courseName.toLowerCase()?.includes(search.toLowerCase()))
  }else{
    diplayedArr = publishedCourses;
  }
  // useEffect(()=>{
