...
function Dragdrop({ size, useFor, onFileDrop }) {
	...
	return (
		<>
			{drop && !drop2 && <FontAwesomeIcon icon={faFile} className="icon2" />}
			{!drop && !drop2 && (
				<img
					id="img"
					style={{ width: '60px', height: '60px' }}
					className="rounded ms-auto m-2"
                          ...
                          alt=''
								...
								<div className="drag-desc">
									{size == 'sm' ? (
										...
										<h4 className="h2 text-center fs-5 text-light">
											{h2text}{' '}
										</h4>
									)}
									<label
										className="browse fs-5"
										htmlFor="upload"
			...
			<button
				className={
					(!drop && drop2) || (drop && drop2)
						? 'd-none'
						: 'btn ps-4 pe-4 p-2 btn btn-outline-warning ms-3'
				}
				onClick={(event) => {
