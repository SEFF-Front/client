...
import { getArticle } from '../redux/reducers/ArticlesSlice';
        ...
        <div class="child-contin">
          <div class="container">
            ...
            <div class="margin"></div>
              ...
              <div >         <div class="text-light mb-5 m-auto" style={{width:"90%"}}>
              <p>{category}</p>
              <div class="line"></div>
              <div class="row">
                <h3 class="col-xs-12 col-md-10"> {title}</h3>
                <small class="col-xs-12 col-md-2 d-flex justify-content-end fw-light">
                  <p><FontAwesomeIcon className='text-secondary' icon={faClock} /> {publish_date && (
  <p>{formatDistanceToNow(new Date(publish_date), { addSuffix: true })}</p>
)}</p>
                </small>
              </div>
            </div>
