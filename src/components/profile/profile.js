...
export default function Profile() {
	...
	useEffect(() => {
		if (user.role !== 'Admin') {
			dispatch(fetchCourses({ role: user?.role?.toLowerCase() }));
		}
	}, [dispatch, user?.role]);
	...
	useEffect(() => {
		if (user && user.role !== 'Admin') {
			setOngoingCourses(
				courses?.filter((course) => course?.status?.includes('going'))
			);
			setEndedCourses(courses?.filter((course) => course?.status?.includes('ended')));
		}
	}, [user, courses]);
	...
	const initialProfileData = {
		...
		userInfo: `${user?.major ?? ''}${user?.major && user?.university ? ', ' : ''}${
			user?.university ?? ''
		}`,
		userId: user?.userId,
		age: user?.age || null,
						...
						<div class="col-sm-12 col-md-3 profilefirst">
							<div>
								...
								<input
									...
									{...register('major')}
									class={editMode ? ' profileinput2' : ' profileinput2 disabled'}
									disabled={!editMode}
								/>
								...
								<div className="my-4">
									...
									<div class=" d-flex flex-column gap-2">
										{ongoingCourses?.length < 1 ? (
											<h5 class="text-white">No Upcoming Exams!</h5>
										) : (
									...
									<div class=" d-flex flex-column gap-2">
										{endedCourses?.length < 1 ? (
											<h5 class="text-white">No finished Exams!</h5>
										) : (
